...
class ProductService {
    ...
    save(product) {
        return new Promise((resolve, reject) => {
            connection.getConnection().query(`INSERT INTO products (name, price, quantity, image)
    ...
    update(product) {
        return new Promise((resolve, reject) => {
            connection.getConnection().query(`UPDATE products
                                              SET name     = '${product.name}',
                                                  price    = ${product.price},
                                                  quantity = '${product.quantity}',
                                                  image    = '${product.image}'
                                              WHERE (id = '${product.id}')`, (err, products) => {
                if (err) {
                    reject(err)
                } else {
    ...
    findByName(name) {
        ...
        return new Promise((resolve, reject) => {
            connection.getConnection().query(`select *
                                              from products
                                              where name like '%${name}%'`, (err, products) => {
                if (err) {
                    reject(err)
                } else {
                    resolve(products)
